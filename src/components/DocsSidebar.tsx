  ...
  return (
    <div 
      ...
      className={cn(
        ...
        isGlass ? "border-white/10" : "border-(--t-border)"
      )}
    >
      {/* Search Bar */}
      <div className="mb-8 relative group px-2">
        <label className={cn(
          ...
          isGlass 
            ...
            : "border-(--t-border) bg-(--t-bg-secondary) hover:border-(--t-border-strong) focus-within:ring-1 focus-within:ring-(--t-accent)/20 shadow-sm"
        )}>
          <Search size={14} className="text-(--t-text-muted) group-focus-within:text-(--t-accent) transition-colors shrink-0" />
          <input
            type="text"
            placeholder="Search docs..."
            value={searchQuery}
            onChange={(e) => setSearchQuery(e.target.value)}
            className="w-full text-[13px] bg-transparent outline-none placeholder:text-(--t-text-muted)/50 font-medium"
          />
        </label>
      </div>
            ...
            return (
              ...
              >
                <h4 className={cn(
                  ...
                  isCategoryActive ? "text-(--t-accent)" : "text-(--t-text-muted)/60"
                )}>
                      ...
                      <li key={section.id} className="px-2">
                        <NavLink
                          ...
                          className={({ isActive }) => cn(
                            ...
                            isActive 
                              ? "active-sidebar-link text-(--t-accent) font-bold bg-(--t-accent)/[0.04]" 
                              : "text-(--t-text-secondary) hover:text-(--t-text) hover:bg-(--t-surface-hover)"
                          )}
                        >
                          {({ isActive }) => (
                            <>
                              {isActive && (
                                <motion.div 
                                  ...
                                  className="absolute inset-0 rounded-lg bg-(--t-accent)/[0.04] border border-(--t-accent)/[0.08] -z-10"
                                  transition={{ type: "spring", stiffness: 400, damping: 30 }}
                                />
                              )}
                              <span className={cn(
                                "w-1 h-1 rounded-full transition-all shrink-0",
                                isActive ? "bg-(--t-accent) scale-125 shadow-[0_0_8px_var(--t-accent)]" : "bg-transparent group-hover:bg-(--t-text-muted)/30"
                              )} />
                              <span className={cn(
