  ...
  <>
    ...
    <div className="overflow-x-auto my-8">
      <table className="w-full text-left border-collapse">
        <thead>
          <tr className="border-b border-(--t-border-strong)">
            <th className="py-2 font-bold uppercase text-[10px] tracking-widest text-(--t-text-muted)">Variable</th>
            <th className="py-2 font-bold uppercase text-[10px] tracking-widest text-(--t-text-muted)">Description</th>
            <th className="py-2 font-bold uppercase text-[10px] tracking-widest text-(--t-text-muted)">Default</th>
          </tr>
        </thead>
        <tbody className="text-sm">
          <tr className="border-b border-(--t-border)">
            <td className="py-2 font-mono">APP_ENV</td>
            <td className="py-2 text-(--t-text-secondary)">environment name (<code>development</code>, <code>production</code>)</td>
            <td className="py-2"><code>development</code></td>
          </tr>
          <tr className="border-b border-(--t-border)">
            <td className="py-2 font-mono">APP_PORT</td>
            <td className="py-2 text-(--t-text-secondary)">The port the HTTP server binds to</td>
            <td className="py-2"><code>8080</code></td>
          </tr>
          <tr className="border-b border-(--t-border)">
            <td className="py-2 font-mono">DATABASE_URL</td>
            <td className="py-2 text-(--t-text-secondary)">Connection string for your DB</td>
            <td className="py-2">—</td>
          </tr>
          <tr className="border-b border-(--t-border)">
            <td className="py-2 font-mono">REDIS_URL</td>
            <td className="py-2 text-(--t-text-secondary)">Connection string for Redis</td>
            <td className="py-2"><code>localhost:6379</code></td>
          </tr>
          <tr className="border-b border-(--t-border)">
            <td className="py-2 font-mono">JWT_SECRET</td>
            <td className="py-2 text-(--t-text-secondary)">Secret used for signing tokens</td>
            <td className="py-2">—</td>
          </tr>
        </tbody>
      </table>
    ...
    <div className="mt-12 py-8 border-t border-(--t-border)">
      ...
      <ul className="list-none pl-0 space-y-2">
        <li>
          ...
          <span className="text-(--t-text-muted) ml-2">— Understand where to put your code.</span>
        </li>
        <li>
          <Link to="/docs/core-concepts/routing" className="text-(--t-accent) no-underline hover:underline font-bold">
            Routing &rarr;
          </Link>
          <span className="text-(--t-text-muted) ml-2">— Set up your first real application routes.</span>
        </li>
        <li>
          <Link to="/docs/development/deployment" className="text-(--t-accent) no-underline hover:underline font-bold">
            App Configuration &rarr;
          </Link>
          <span className="text-(--t-text-muted) ml-2">— Learn more about customizing <code>astra.config.yaml</code>.</span>
        </li>
      </ul>
    </div>
  </>
